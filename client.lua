        ...
        Citizen.Wait(1)
            if not IsPedInAnyVehicle(PlayerPedId(), true) then
                for i=1, #Config.pos, 1 do 
                    ...
                    if distance <= 3 then
                        Draw3DText(coords2.x, coords2.y, coords2.z + 1.2, '[E] - Check in for $' .. Config.price)
                            if IsControlJustReleased(0, 54) then
                             DisableControlAction(0, 54, true)
                            if (GetEntityHealth(PlayerPedId()) >= 200) then
                                exports['mythic_notify']:SendAlert('error', 'You dont need medical attention')
                            elseif (GetEntityHealth(PlayerPedId()) <= 200) or IsEntityDead(PlayePedId()) then
                                    exports['mythic_progbar']:Progress({
                                        name = "Hospital Revive",
                                        duration = 1000,
                                        label = "Checking In",
                                        useWhileDead = true,
                                        canCancel = true,
